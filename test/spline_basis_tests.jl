...
@testset "1D B-spline basis evaluation with Dirichlet boundary conditions" begin
    ...
    @test evaluate_basis_derivative(s, [0.00], 1) == [+4.0]
    @test evaluate_basis_derivative(s, [0.00], 2) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.00], 3) == [ 0.0]
    ...
    @test evaluate_basis_derivative(s, [0.50], 1) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.50], 2) == [-4.0]
    @test evaluate_basis_derivative(s, [0.50], 3) == [+4.0]
    ...
    @test evaluate_basis_derivative(s, [1.00], 1) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 2) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 3) == [-4.0]


    s = SplineND(1, 3, 0:0.25:1, :Dirichlet, GaussLegendreQuadrature(2))

    @test evaluate_basis_derivative(s, [0.00], 1) == [+8.0]
    @test evaluate_basis_derivative(s, [0.00], 2) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.00], 3) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.00], 4) == [ 0.0]
    ...
    @test evaluate_basis_derivative(s, [0.50], 2) == [-4.0]
    @test evaluate_basis_derivative(s, [0.50], 3) == [+4.0]
    @test evaluate_basis_derivative(s, [0.50], 4) == [ 0.0]
    ...
    @test evaluate_basis_derivative(s, [1.00], 1) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 2) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 3) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 4) == [-8.0]
...
@testset "1D B-spline basis derivative evaluation with periodic boundary conditions" begin
    ...
    @test evaluate_basis_derivative(s, [0.00], 1) == [-4.0]
    @test evaluate_basis_derivative(s, [0.00], 2) == [+4.0]
    @test evaluate_basis_derivative(s, [0.00], 3) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.00], 4) == [ 0.0]
    ...
    @test evaluate_basis_derivative(s, [0.50], 1) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.50], 2) == [ 0.0]
    @test evaluate_basis_derivative(s, [0.50], 3) == [-4.0]
    @test evaluate_basis_derivative(s, [0.50], 4) == [+4.0]
    ...
    @test evaluate_basis_derivative(s, [1.00], 2) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 3) == [ 0.0]
    @test evaluate_basis_derivative(s, [1.00], 4) == [-4.0]
